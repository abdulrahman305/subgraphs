...
export function getUSDPriceWithoutDecimals(
  ...
  return getUSDPrice(tokenAddr)
    .times(amount)
    .div(BIGINT_TEN.pow(token.decimals as u8).toBigDecimal());
}

export function getUSDPrice(tokenAddr: Address): BigDecimal {
  const yetiControllerContract = YetiController.bind(
    Address.fromString(YETI_CONTROLLER),
  );
...
export function getAsssetsUSD(
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    totalAmount = totalAmount.plus(
      getUSDPriceWithoutDecimals(token, amount.toBigDecimal()),
    );
  }
  return totalAmount;
}
